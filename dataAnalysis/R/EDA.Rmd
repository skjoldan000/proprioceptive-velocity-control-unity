...
plot_endpoints2 <- trials %>%
  filter(blockNumber == 3) %>%
  filter(visualXOffset == 0) %>%
  ggplot(aes(x = trueInput.x, y = trueInput.z, color = factor(visualZOffset))) +
  targetOut +
  geom_point(size = 3) +
  stat_ellipse(level = 0.95, size = 2) +
  coord_fixed(ratio = 1)+ theme(legend.position = "bottom")
...
plot_endpoints2 <- trials %>%
  ...
  # targetHome +
  targetOut +
  geom_point(aes(shape = factor(visualXOffset)), size = 3) +
  stat_ellipse(level = 0.95, size = 2) +
  coord_fixed(ratio = 1)+ theme(legend.position = "bottom")
...
plot_endpoints2 <- trials %>%
  ...
  # targetHome +
  targetOut +
  geom_point(aes(shape = factor(visualZOffset)), size = 3) +
  stat_ellipse(level = 0.95, size = 2) +
  coord_fixed(ratio = 1)+ theme(legend.position = "bottom")
